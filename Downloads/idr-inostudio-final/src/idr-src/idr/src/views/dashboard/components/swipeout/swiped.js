...
var Swiped = function (o) {
  ...
  this.leftAction = o.leftAction;
  this.rightAction = o.rightAction;
  ...
  this.onLongSwipe = typeof o.onLongSwipe === 'function' ? o.onLongSwipe: fn;
  this.onLongSwipeComplete = typeof o.onLongSwipeComplete === 'function' ? o.onLongSwipeComplete: fn;
...
Swiped.prototype.touchEnd = function (e) {
  ...
  if (this.dir * this.delta > this.tolerance || new Date() - this.startTime < this.time) {
    const itemWidth = this.elem.parentNode.offsetWidth;

    if (this.overswipe) {
      this.animation(this.dir * itemWidth, 150);
      setTimeout(() => {
        this.onLongSwipeComplete(this.overswipe);
        this.close();
      }, 200);
    } else {
      this.open();
  ...
  this.elem.style.removeProperty(cssProps.transition);
  this.leftAction.style.removeProperty(cssProps.transition);
  this.rightAction.style.removeProperty(cssProps.transition);
...
Swiped.prototype.move = function () {
  ...
  if (deltaAbs > this.width) {
    // linear deceleration
    this.delta = this.dir * (this.width + (deltaAbs - this.width) / 1.1);
  }

  // if swipe is more than half of item's width, activate long swipe
  if (Math.abs(this.delta) > this.elem.parentNode.offsetWidth / 2) {
    this.overswipe = this.dir;
    this.onLongSwipe(this.overswipe, true);
  } else{
    this.overswipe = null;
  }
...
Swiped.prototype.animation = function (x, duration) {
  ...
  this.elem.style[cssProps.transitionDuration] = duration + 'ms';
  this.elem.style[cssProps.transform] = 'translate3d(' + x + 'px, 0px, 0px)';
  if (this.leftAction) {
    this.leftAction.style[cssProps.transitionDuration] = duration + 'ms';
    this.leftAction.style[cssProps.transform] = 'translate3d(' + x + 'px, 0px, 0px)';
  }
  if (this.rightAction) {
    this.rightAction.style[cssProps.transitionDuration] = duration + 'ms';
    this.rightAction.style[cssProps.transform] = 'translate3d(' + x + 'px, 0px, 0px)';
  }
